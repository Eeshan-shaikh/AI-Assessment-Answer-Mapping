...
export function AnswerViewer({ data, selectedQuestionId }: AnswerViewerProps) {
  ...
  useEffect(() => {
    if (answer && answer.pages && answer.pages.length > 0) {
      if (!answer.pages.includes(currentPage)) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Toolbar */}
      <div className="h-12 bg-gray-900 text-gray-300 flex items-center justify-between px-4 shrink-0 z-10 shadow-sm">
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-3 bg-gray-800 rounded-md px-2 py-1">
            <button onClick={() => setScale(s => Math.max(0.5, s - 0.25))} className="hover:text-white transition-colors"><ZoomOut className="w-4 h-4" /></button>
            <span className="text-xs font-medium w-10 text-center">{Math.round(scale * 100)}%</span>
            <button onClick={() => setScale(s => Math.min(3, s + 0.25))} className="hover:text-white transition-colors"><ZoomIn className="w-4 h-4" /></button>
          </div>
          ...
          <div className="flex items-center gap-3 bg-gray-800 rounded-md px-2 py-1">
            ...
            <button 
              ...
              className="hover:text-white disabled:opacity-30 transition-colors"
              disabled={currentPage >= numPages && isPdf}
      ...
      <div className="flex-1 overflow-auto bg-gray-200 p-4 md:p-8 flex justify-center relative">
        ...
        {/* Container wrapping scaled content so scrollbars work properly */}
        <div 
          ...
          style={{ 
            ...
            marginBottom: `${(scale - 1) * 1131}px`, // Fix layout issues when scaled up
            marginRight: `${(scale - 1) * 800}px`
          }}
        >
          {/* Page Container */}
          <div 
            className="bg-white shadow-xl relative bg-cover bg-center bg-no-repeat w-full h-full"
            style={{ 
          ...
          >
            ...
            {!fileUrl && (
              <>
                ...
                ></div>
                <div className="absolute left-24 top-0 bottom-0 w-px bg-red-400 opacity-40 pointer-events-none"></div>
                
                <div className="absolute inset-0 p-12 pl-32 font-serif text-blue-900/80 text-xl leading-[40px] pt-[85px] pointer-events-none">
